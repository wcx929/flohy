...
class StoreSeckill extends BaseModel
{
    ...
    {
        $time = time();
        $info = self::alias('n')->join('store_product c', 'c.id=n.product_id')->where('n.id', $id)->where('c.is_show',1)->where('c.is_del',0)->where('n.is_del', 0)->where('n.status', 1)->where('n.start_time', '<', $time)->where('n.stop_time', '>', $time)->field('n.*,SUM(c.sales+c.ficti) as total')->find();
        if ($info['id']) {
            return $info;
        } else {
            return [];
