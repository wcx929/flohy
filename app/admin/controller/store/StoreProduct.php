...
class StoreProduct extends AuthController
{
    ...
    public function save($id)
    {
        ...
        if ($id) {
            ...
            if ($data['spec_type'] == 0) {
                $info = StoreProductAttrValue::where('product_id', $id)->where('type', 0)->select();
                if (count($info) > 1) {
                    StoreProductAttr::where('product_id', $id)->where('type', 0)->delete();
                    StoreProductAttrResult::where('product_id', $id)->where('type', 0)->delete();
                    StoreProductAttrValue::where('product_id', $id)->where('type', 0)->delete();
                }
                $attr = [
                    [
                        'value' => '规格',
