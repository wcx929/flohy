...
class COS
{
    ...
    protected static function autoInfo()
    {
        if (($storageRegion = Cache::get('storageRegion')) && ($storageName = Cache::get('storageName')) && ($uploadUrl = Cache::get('uploadUrl')) && ($accessKey = Cache::get('accessKey')) && ($secretKey = Cache::get('secretKey'))) {
            ...
            self::$accessKey = trim(sys_config('accessKey'));
            self::$secretKey = trim(sys_config('secretKey'));
            self::$uploadUrl = trim(sys_config('uploadUrl')) . '/';
            self::$storageName = trim(sys_config('storage_name'));
            self::$storageRegion = trim(sys_config('storage_region'));
